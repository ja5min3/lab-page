# 先進顯微鏡技術實驗室 · 內部知識庫

明志科技大學化工系 化306-1。指導教授：杜鶴芸。

線上網址：<https://ja5min3.github.io/lab-page/>

## 這個 repo 是什麼

單一自持檔案 `index.html`，沒有建置流程、沒有相依套件、沒有外部字型或 CDN，
存起來離線也能開。GitHub Pages 從 `main` 分支自動部署，推上去約 40 秒後生效。

## 來源與同步

- **這個 repo 是部署來源。** 線上網頁一律從 `main` 的 `index.html` 產生，
  要改網頁就改這裡。
- **改動走分支，不直接推 `main`。** 在分支上修改並完成驗證後，
  **必須取得擁有者明確確認，才能合併進 `main`。**
  合併即等於更新線上網站，沒有例外。
- **vault 保留同步鏡像。** 第二大腦（Obsidian）內的「實驗室網頁.html」是這份檔案的
  鏡像，方便離線閱讀與版本留存。**它是鏡像，不是編輯來源。**
- 兩邊要保持一致：改完 repo 後，把 `index.html` 覆蓋回 vault 那份鏡像。
  反過來從 vault 覆蓋 repo 之前，先確認 vault 那份確實比 `main` 新，
  否則會把線上版本蓋掉。

## 頁面結構

單檔內用 hash 路由分頁，網址可以直接分享到某一區：

| 網址 | 內容 |
|---|---|
| `#home` | 首頁，七張入口卡 |
| `#print` | 3D 列印區總覽 |
| `#d-h2d` `#d-ender` `#d-s050` | 三台機器的操作細節 |
| `#d-exam` | 使用考核制度 |
| `#inventory` | 耗材盤點 |
| `#safety` `#d-check` | 安全與檢查、自主檢查八項 |
| `#waste` `#buy` `#members` `#rules` | 廢液、採購、成員、其他規則 |

路由是漸進增強的：CSS 預設顯示所有區塊，JS 啟動後才隱藏非當前分頁。
**停用 JavaScript 時會退回一頁到底的長捲頁，不會變空白。**

## 耗材盤點的資料存在哪裡

有兩種模式，由 `index.html` 裡的 `INVENTORY_SYNC_URL` 這個常數決定。

### 模式一：只存本機（`INVENTORY_SYNC_URL` 留空）

資料存在**瀏覽器的 localStorage**，key `lab-inventory-v1`，綁定瀏覽器與網址。
只有同一台電腦同一個瀏覽器看得到，換人換機器不會同步，清瀏覽器資料就沒了。
備份與交接靠頁面上的「匯出 CSV」，到新的地方用「匯入 CSV」讀回來。

### 模式二：全實驗室共用 Google 試算表（填入網址）

資料正本在共用試算表，localStorage 降級成離線快取。
**讀取公開、寫入要密碼**；斷線時照常操作，畫面顯示「離線 · N 筆待同步」，回線自動補送。

架構是靜態頁 → Apps Script 網頁應用程式 → 試算表。頁面送出的是**單一操作**
（`upsert` / `adjust` / `delete` / `bulkUpload` / `clearAll`）而不是整表覆蓋，
所以兩個人同時改不同品項不會互相蓋掉；伺服器端用 `LockService` 序列化，
並記住最近 300 個 `opId` 擋掉網路不穩造成的重送（`adjust` 不是冪等操作，重送會多加）。

#### 部署步驟

1. **確認試算表是原生格式。** 標題右邊若有綠色 **XLSX** 標記，代表是上傳的 Excel，
   Apps Script 用不了 —— 先「檔案 → 另存為 Google 試算表」，改用轉出來的那份。
2. 在試算表裡開「擴充功能 → Apps Script」，把 `apps-script/Code.gs` 全部貼進去。
3. 改 `setWriteToken()` 裡的密碼字串，在編輯器選這個函式**執行一次**（會要求授權）。
   密碼存進 Script Properties，不會留在程式碼裡。
4. 「部署 → 新增部署作業 → 網頁應用程式」，執行身分選**我**，
   誰可以存取選**任何人**，部署後複製網址。
5. 把網址填進 `index.html` 的 `INVENTORY_SYNC_URL`，推上分支，確認後合併 `main`。

工作表叫「庫存」，第一次執行會自動建立，欄位：
`id | 類別 | 品項 | 顏色／規格 | 數量 | 單位 | 備註 | 最後更新 | 更新者`。

#### 換密碼

改 `setWriteToken()` 裡的字串再執行一次即可，不用重新部署。
成員端下次寫入會收到「密碼不正確」提示並重新詢問。

#### 已知取捨

- **讀取是公開的。** 網頁應用程式網址寫在公開的頁面原始碼裡，拿到網址的人都能讀庫存。
  寫入密碼由使用者自己輸入、存在各自瀏覽器，**不會出現在原始碼**，所以寫入保護是有效的。
- 「清空」在同步模式下清的是**全實驗室共用的試算表**，按鈕上的確認訊息會特別說明。
- 每次寫入是一次網路往返，大約數百毫秒，比純本機慢。

改動 `index.html` 時，以下不能更名，否則使用者已輸入的庫存會讀不到或功能失效。
三類的壞法不一樣，改之前先看清楚是哪一類：

**localStorage key** —— 換掉等於所有既有庫存讀不回來

- `lab-inventory-v1` —— 庫存資料（同步模式下是離線快取）
- `lab-inventory-queue-v1` —— 待送出的操作，換掉會讓離線期間的修改遺失
- `lab-inventory-token-v1` —— 寫入密碼，換掉只會多問一次密碼
- `lab-inventory-user-v1` —— 更新者名字，可有可無

**JS 以 `getElementById` 抓的 DOM id** —— 換掉會直接壞掉或靜默失效

- 表單與工具列：`inv-form` `inv-cat` `inv-name` `inv-spec` `inv-qty` `inv-unit`
  `inv-note` `inv-submit` `inv-cancel` `inv-search` `inv-filter`
  `inv-export` `inv-import` `inv-import-btn` `inv-clear`
- 清單與統計：`inv-body` `inv-empty` `inv-summary`
- 標尺與首頁入口卡上的品項數：`inv-meta` `home-inv-meta`
  （這兩個是「找不到就跳過」的寫法，改名不會報錯，只會安靜地不再更新，特別容易漏掉）

**HTML `list=` 指向的 datalist id** —— 換掉輸入建議會消失，不會報錯

- `inv-name-list` `inv-spec-list` `inv-unit-list`

**CSS 選擇器用到的 id** —— 換掉排版會跑掉

- `inv-table`（`#inv-table` 有三條規則：最小寬度、儲存格對齊、數量欄字體）

**同步模式的說明區塊** —— 換掉切換不到正確的說明文字

- `inv-note-local`（只存本機時顯示）`inv-note-sync`（接上試算表時顯示）

## 設計慣例

- 配色深海軍藍，色票集中在 `:root` 的 CSS 變數
- **等寬字只給「量」用**：數字、單位、規格、統計走 `--mono`，中文敘述走黑體
- 每個區塊標題下的「標尺」右側標出該區真正的量（3 台機器、8 項檢查…），
  耗材盤點那條會即時反映目前品項數
- 提示框預設用安靜的藍（`.note`），琥珀色（`.note--risk`）只留給真正有風險的地方
- 標題不放 emoji
- 最小字級 14px（根字級 18px，即 `.78rem`）——這頁常常是站在機器旁邊用手機看的
